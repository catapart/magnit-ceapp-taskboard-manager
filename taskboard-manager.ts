...
import componentStyle from './taskboard-manager.component.css?raw';
// html
import html from './taskboard-manager.component.html?raw';
// icons
import { Icons } from './assets/icons/icons.asset';

// component definitions
import '../../simple/editable-list/editable-list.component';
...
import '../../form/form-field/form-field.component';
import '../../form/image-input/image-input.component';
import '../../form/file-input/file-input.component';
import './components/taskboard-fields/taskboard-fields.component';
import './components/import-manager/import-manager.component';
import '../action-history/action-history.component';
import '../../simple/message-card/message-card.component'
// components
...
import { HistoryEntryComponent } from '../action-history/history-entry/history-entry.component';
...
import { EditableListComponent } from '../../simple/editable-list/editable-list.component';
import { ATTRIBUTENAME_REVERSED, ActionHistoryComponent, CLASSNAME_ACTIVE } from '../action-history/action-history.component';
import { DataChannel } from './data/channels/data.channel';
import { MessageCardComponent, MessageCardEvent, MessageCardType } from '../../simple/message-card/message-card.component';
...
import { addRouteHandlers } from './handlers/route.handlers';
...
{ 
    ...
    updateListRecord: (taskListComponent: TaskListComponent) => void,
    duplicateList: (target: HTMLElement, list: TaskListRecord, settings: TaskSettingsRecord) => void,

    registerTaskCard: (card: TaskCardComponent, listId: string, order: number) => void,
    updateTaskRecord: (taskComponent: TaskCardComponent, parentList: TaskListComponent) => void,
    deleteTaskRecord: (taskComponent: TaskCardComponent) => void,
    updateTaskRecordsAfterMove: (target: TaskCardComponent, parent: TaskListComponent) => void,
...
export class TaskboardManagerComponent extends HTMLElement
{
    ...
    */
    ...
    async openBoard(id: string)
    {
        ...
        await this.getPart<PathRouterComponent>('app-router').navigate(`board/${id}`);
    }
    async closeBoard()
    {
        await this.findPart<PathRouterComponent>('app-router').navigate('/' + window.location.hash);
    ...
    async openBoardSettings(id: string)
    {
        ...
        if(board == null)
        {
            MessageCardComponent.notify(`No board found with the target id (${id}).`, 
        ...
        {
            MessageCardComponent.notify(`An error occurred accessing task settings data.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
    ...
    #addUndoNotification(message: string, entryId: string)
    {
        ...
        const notification = MessageCardComponent.prepare(content, this.findPart('notifications'), { type: MessageCardType.Success, heading: "Success!" });
        messageButton.addEventListener('click', () =>
        {
            const entry = this.getPart<ActionHistoryComponent>('action-history').querySelector(`[data-entry-id="${entryId}"]`) as HTMLElement;
            if(entry == null)
            {
                MessageCardComponent.notify(`An error occurred restoring a record. The record was not restored`,
                this.findPart('notifications'), { type: MessageCardType.Error });
                return;
            }
            this.getPart<ActionHistoryComponent>('action-history').reverseEntry(entry);
    ...
    async addTask(listId: string)
    {
        ...
        const newCard = new TaskCardComponent();
        list.append(newCard);
    ...
    async undo()
    {
        this.findPart<ActionHistoryComponent>('action-history').back();
    }
    async redo()
    {
        this.findPart<ActionHistoryComponent>('action-history').forward();
    }
    ...
    async #init()
    {
        ...
        const urlRequestedRoute = window.location.pathname + window.location.hash;
        await this.getPart<PathRouterComponent>('app-router').navigate(urlRequestedRoute);
    ...
    async #refreshBoards()
    {
        ...
        const menuItems: RouteLinkComponent[] = [];
        const collectionItems: CaptionedThumbnailComponent[] = [];
    ...
    {
        const element = new CaptionedThumbnailComponent();
    ...
    async #refreshRecentBoards()
    {
        ...
        const recentBoards = this.getPart<EditableListComponent>('recent-boards');
        ...
        for(let i = 0; i < boards.length; i++)
        {
            ...
            element.setAttribute('is', 'route-link');
    ...
    async #renderBoard(id: string)
    {
        ...
        if(board == null)
        {
            this.findPart<PathRouterComponent>('app-router').navigate('/');
            MessageCardComponent.notify(`No board found with the target id (${id}). Navigated back to Welcome page.`, 
        ...
        const taskBoard = this.findPart<TaskBoardComponent>('task-board');
        taskBoard.innerHTML = '';
    ...
    async #renderBoardBackground(board: TaskBoardRecord)
    {
        ...
        if(board.backgroundImageId != null && board.backgroundImageId.trim() != "")
        {
            ...
            if(backgroundImageUrl == null)
            {
                ...
                {
                    MessageCardComponent.notify(`No image found with the target id (${board.backgroundImageId}).`, 
                    this.getPart('notifications'), { type: MessageCardType.Warn });
    ...
    async #updateBoardSettings()
    {
        ...
        { 
            MessageCardComponent.notify(`An error occurred saving a task board.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
        ...
        const imageValue = boardFields.findPart<ImageInputComponent>('background-image').value;
        ...
        MessageCardComponent.notify(`The board settings have been saved successfully!`, 
    ...
    // lists
    async #loadLists(board: TaskBoardComponent, tasks: TaskRecord[])
    {
        const boardId = board.dataset.boardId;
        if(boardId == null)
        {
            MessageCardComponent.notify(`An error occurred loading the board. Navigated back to Welcome page.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
        ...
        for(let i = 0; i < lists.length; i++)
        {
            ...
            if(settings == null)
            {
                MessageCardComponent.notify(`An error occurred loading a list's settings. Some settings may not be displayed properly.`, 
                this.getPart('notifications'), { type: MessageCardType.Warn });
                console.warn(new Error(`Unable to find settings from list's taskSettingsId.`));
            }
            const element = new TaskListComponent();
    ...
    #loadListTasks(taskList: TaskListComponent, tasks: TaskRecord[])
    {
        const listId = taskList.dataset.tasklistId;
        for(let i = 0; i < tasks.length; i++)
        {
            const taskElements: TaskCardComponent[] = [];
            if(tasks[i].listId == listId)
            {
                const task = tasks[i];
                if(task.deletedTimestamp != undefined) { continue; }
                const taskElement = new TaskCardComponent();
    ...
    async #updateListRecord(taskListComponent: TaskListComponent)
    {
        ...
        if(id == null)
        {
            MessageCardComponent.notify(`An error occurred saving a task list.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
    ...
    // tasks
    async #getOrderedTasks(tasklist: TaskListComponent)
    {
    ...
    async #getTaskFromComponent(taskComponent: TaskCardComponent)
    {
        const channel = this.#getChannel(this.#data.tasks, TASK_ERROR_MESSAGE, 'danger');
        const id = taskComponent.dataset.taskId;
        if(id == null)
        {
            MessageCardComponent.notify(`An error occurred identifying a task.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
    ...
    async #registerTaskCard(card: TaskCardComponent, listId: string, order: number)
    {
    ...
    async #updateTaskRecord(taskComponent: TaskCardComponent, parentList: TaskListComponent)
    {
        const channel = this.#getChannel(this.#data.tasks, TASK_ERROR_MESSAGE, 'danger');
        ...
        if(listId == null)
        {
            MessageCardComponent.notify(`An error occurred saving a task.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
        ...
        const tasks = [...parentList.querySelectorAll('task-card')] as TaskCardComponent[];
    ...
    async #deleteTaskRecord(taskComponent: TaskCardComponent)
    {
        const channel = this.#getChannel(this.#data.tasks, TASK_ERROR_MESSAGE, 'danger');
        const id = taskComponent.dataset.taskId;
        if(id == null)
        {
            MessageCardComponent.notify(`An error occurred deleting a task.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
    ...
    async #updateTaskRecordsAfterMove(target: TaskCardComponent, parent: TaskListComponent)
    {
        await this.#updateTaskRecord(target, parent);

        if(this.#data.tasks == null)
        {
            MessageCardComponent.notify(`An error occurred moving a task.`, 
            this.getPart('notifications'), { type: MessageCardType.Error });
    ...
    #initTaskCard(card: TaskCardComponent, task: TaskRecord)
    {
    ...
    async #refreshActionHistory()
    {
        ...
        let entries: HistoryEntryComponent[] = [];
        let activeEntry: HistoryEntryComponent | null = null;
        for(let i = 0; i < records.length; i++)
        {
            ...
            {
                entry.classList.add(CLASSNAME_ACTIVE); 
                activeEntry = entry;
                continue;
            }
            if(activeEntry != null)
    ...
    {
        const element = new HistoryEntryComponent();
    ...
    async #openImportManager(data: any)
    {
        ...
        this.findPart<PathRouterComponent>('app-router').navigate('#import');
    ...
    #getConfirmation(message: string, type: 'info'|'warn'|'danger' = 'info')
    {
        ...
        this.getPart<PathRouterComponent>('confirmation-router').navigate(type);
...
{
    customElements.define(COMPONENT_TAG_NAME, TaskboardManagerComponent);
}